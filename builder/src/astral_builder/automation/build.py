    ...
    *,
    ...
    manifest_files.append(
        ManifestFile.from_paths(
            ...
            source_transport=lang_source_transport,
            source_download_url=_download_url(source_asset_base_url, lang_source_transport.name),
        )
    )
    ...
    str_source_transport = _package_payload(str_item.path, originals, str_release_name)
    manifest_files.append(
        ManifestFile.from_paths(
            str_out,
            str_transport,
            ...
            source_transport=str_source_transport,
            source_download_url=_download_url(source_asset_base_url, str_source_transport.name),
        )
    )
    ...
    tmp_source_transport = _package_payload(tmp_item.path, originals, tmp_release_name)
    manifest_files.append(
        ManifestFile.from_paths(
            tmp_out,
            tmp_transport,
            ...
            source_transport=tmp_source_transport,
            source_download_url=_download_url(source_asset_base_url, tmp_source_transport.name),
        )
    )
    ...
    if config.legacy_font_name is not None and config.platform != "android":
        ...
        legacy_source_transport = _package_payload(
            Path(legacy_data_path), originals, legacy_release_name
        )
        manifest_files.append(
            ManifestFile.from_paths(
                legacy_out,
                ...
                source_transport=legacy_source_transport,
                source_download_url=_download_url(
                    source_asset_base_url, legacy_source_transport.name
                ),
            )
        )
