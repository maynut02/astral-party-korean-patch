...
final class MobilePatcherRelease {
    ...
    static MobilePatcherRelease parse(String json) throws JSONException {
        ...
        if (versionCode != expectedVersionCode) {
            throw new JSONException("Mobile Patcher versionCode가 version과 일치하지 않습니다.");
        }

        String downloadUrl = root.getString("downloadUrl");
        String expectedUrl = "https://github.com/" + REPOSITORY
                + "/releases/download/mobile-patcher-v" + version
                + "/AstralMobilePatcher-v" + version + ".apk";
        if (!expectedUrl.equals(downloadUrl)) {
            throw new JSONException("신뢰하지 않는 Mobile Patcher APK 다운로드 주소입니다.");
        }

        String sha256 = root.getString("sha256").toLowerCase(Locale.ROOT);
        if (!sha256.matches("[0-9a-f]{64}")) {
            throw new JSONException("잘못된 Mobile Patcher SHA-256 값입니다.");
        }

        long size = root.getLong("size");
        if (size <= 0) {
            throw new JSONException("잘못된 Mobile Patcher APK 크기입니다.");
        }
    ...
    private static int versionCodeFrom(String version) throws JSONException {
        if (!version.matches("[0-9]+\\.[0-9]+\\.[0-9]+")) {
            throw new JSONException("Mobile Patcher version이 SemVer 형식이 아닙니다.");
        }
        ...
        try {
            ...
            if (major > 2100 || minor > 999 || patch > 999) {
                throw new JSONException("Mobile Patcher version 범위가 올바르지 않습니다.");
            }
            long code = major * 1_000_000L + minor * 1_000L + patch;
            if (code < 1 || code > 2_100_000_000L) {
                throw new JSONException("Mobile Patcher versionCode 범위가 올바르지 않습니다.");
            }
            return (int) code;
        } catch (NumberFormatException error) {
            throw new JSONException("Mobile Patcher version 숫자를 해석할 수 없습니다.");
        }
    }
}
