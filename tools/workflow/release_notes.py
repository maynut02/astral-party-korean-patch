    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"- 버전: `{version}`",
            "- 대상: Windows x64",
            "- `AstralAutoPatcher.exe`를 내려받아 바로 실행하면 됩니다.",
            "- Steam 패치 설치/제거와 INT_ANDROID 설치/업데이트를 지원합니다.",
            "",
            "## 확인",
            "",
            f"- SHA-256: `{sha256}`",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "- 일반 APK 설치 대신 `AstralAutoPatcher`를 사용하세요.",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    autopatcher = sub.add_parser("autopatcher", parents=[common])
    autopatcher.add_argument("--version", required=True)
    autopatcher.add_argument("--sha256", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    elif args.kind == "autopatcher":
        text = render_autopatcher_notes(
