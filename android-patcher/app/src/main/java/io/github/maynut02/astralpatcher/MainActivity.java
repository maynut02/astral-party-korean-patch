...
public final class MainActivity extends Activity {
    ...
    private final ServiceConnection installerServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            GameInstallRequest request = pendingGameInstall;
            pendingGameInstall = null;
            if (request != null) {
                installGameWithBoundService(request);
            }
        }
    ...
    private void updateShizukuUi() {
        ...
        if (Shizuku.checkSelfPermission() != PackageManager.PERMISSION_GRANTED) {
            shizukuStatus.setText("실행 중 · AndroidPatcher 권한이 필요합니다.");
    ...
    private void fetchLatestMobilePatcherIndex() {
        executor.execute(() -> {
            try {
                ...
                appendLog("AndroidPatcher 업데이트 확인 실패"
    ...
    private void downloadLatestMobilePatcher() {
        ...
        setUpdateBusy(true, "AndroidPatcher " + release.version + " APK를 다운로드하는 중입니다.");
        appendLog("AndroidPatcher 업데이트 다운로드 시작: " + release.downloadUrl);
        executor.execute(() -> {
            try {
                ...
                if (!release.sha256.equals(actualHash)) {
                    apk.delete();
                    throw new IllegalStateException("AndroidPatcher APK SHA-256 검증에 실패했습니다.");
                }
                verifyMobilePatcherApk(apk, release);
                appendLog("AndroidPatcher " + release.version + " 다운로드 및 검증 완료");
                runOnUiThread(() -> {
                ...
                showError("AndroidPatcher 업데이트 다운로드에 실패했습니다.", error);
            }
        });
    }
    ...
    private void handleShizukuAction() {
        ...
        if (Shizuku.checkSelfPermission() != PackageManager.PERMISSION_GRANTED) {
            if (Shizuku.shouldShowRequestPermissionRationale()) {
                appendLog("Shizuku 앱에서 AndroidPatcher 권한을 허용한 뒤 다시 시도하세요.");
                openPackage(ShizukuRelease.PACKAGE_NAME);
            } else {
    ...
    private void downloadLatestGame() {
        if (!isShizukuReady()) {
            appendLog("게임 설치에는 실행 중인 Shizuku와 AndroidPatcher 권한이 필요합니다.");
            updateActionAvailability();
            return;
        }
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                PackageInfo apkInfo = verifyApkPackage(
                        apk, DistributionIndex.PACKAGE_NAME, index.gameVersion);
                appendLog("게임 APK 패키지 검증 완료: " + DistributionIndex.PACKAGE_NAME
                        + " / " + index.gameVersion + " (versionCode="
    ...
    private void installGameWithBoundService(GameInstallRequest request) {
        ...
        executor.execute(() -> {
            try {
                ...
                String serviceProtocol = serviceInfo == null
                        ? null
                        : serviceInfo.split(" ", 2)[0];
                if (!INSTALLER_SERVICE_PROTOCOL.equals(serviceProtocol)) {
                    throw new IllegalStateException(
    ...
    private void verifyInstalledGame(GameInstallRequest request) throws Exception {
        ...
        for (int attempt = 1; attempt <= INSTALL_VERIFY_ATTEMPTS; attempt++) {
            ...
            if (installedInfo != null) {
                lastVersion = installedInfo.versionName;
                lastVersionCode = installedInfo.getLongVersionCode();
                try {
                    lastInstaller = installedGameInstaller();
                    appendLog("설치 결과 확인 " + attempt + "/" + INSTALL_VERIFY_ATTEMPTS
                            + ": version=" + lastVersion + ", versionCode=" + lastVersionCode
            ...
            if (attempt < INSTALL_VERIFY_ATTEMPTS) {
                Thread.sleep(INSTALL_VERIFY_DELAY_MS);
            }
        }
    ...
    private PackageInfo verifyApkPackage(
            File apk, String expectedPackage, String expectedVersion) throws Exception {
    ...
    private void verifyMobilePatcherApk(File apk, MobilePatcherRelease release) throws Exception {
        ...
        if (info == null || !getPackageName().equals(info.packageName)) {
            throw new IllegalStateException("다운로드한 AndroidPatcher APK packageName이 예상과 다릅니다.");
        }
        if (!release.version.equals(info.versionName)) {
            throw new IllegalStateException(
                    "다운로드한 AndroidPatcher APK 버전이 배포 정보와 다릅니다: " + info.versionName);
        }
        if (info.getLongVersionCode() != release.versionCode) {
            throw new IllegalStateException(
                    "다운로드한 AndroidPatcher APK versionCode가 배포 정보와 다릅니다: "
                            + info.getLongVersionCode());
        }
        if (info.getLongVersionCode() <= BuildConfig.VERSION_CODE) {
            throw new IllegalStateException("현재 버전보다 새로운 AndroidPatcher APK가 아닙니다.");
        }
        ...
        if (!hasSameCurrentSigner(current, info)) {
            throw new IllegalStateException("다운로드한 AndroidPatcher APK 서명이 현재 앱과 다릅니다.");
        }
    }
    ...
    private String installedGameVersion() {
        PackageInfo info = installedGamePackageInfo();
        return info == null ? null : info.versionName;
    }

    private PackageInfo installedGamePackageInfo() {
        try {
            return getPackageManager().getPackageInfo(DistributionIndex.PACKAGE_NAME, 0);
        } catch (PackageManager.NameNotFoundException error) {
            return null;
        }
    }
    ...
    private static final class GameInstallRequest {
        final File apk;
        final String expectedVersion;
        final long expectedVersionCode;

        GameInstallRequest(File apk, String expectedVersion, long expectedVersionCode) {
            this.apk = apk;
