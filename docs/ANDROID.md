...
게임 실행 중에는 Addressables를 수정하지 않습니다. `UpdateWatcher`가 최초 리소스 다운로드 또는 이후 업데이트의 새 catalog/필요 Bundle 준비 완료를 감지하면 `한글패치 준비 완료` 화면과 `게임 종료` 버튼을 표시합니다. 버튼은 앱 프로세스를 완전히 종료하고, 다음 실행의 `BootstrapActivity`가 patch를 다운로드·적용한 뒤 게임을 시작합니다.
