...
게임 실행 시 `release-index.json`은 한 번만 확인합니다. 새 index entry에는 watcher가 필요한 `addressablesPaths`가 함께 기록되며, 기존 index처럼 이 필드가 없을 때만 최신 후보 manifest 하나를 실행 시 미리 읽어 호환합니다. 게임 실행 후 `UpdateWatcher`는 네트워크에 접근하지 않고 로컬 catalog를 1초 간격으로 확인합니다. 대응 patch가 실행 시 snapshot에 있으면 필요한 cache 경로에 `FileObserver`를 붙여 `CREATE`/`MOVED_TO`/`CLOSE_WRITE` 이벤트를 즉시 받습니다. observer 등록 직후와 이벤트 누락 복구 때는 target 파일이 존재하고 현재 프로세스에서 쓰기용 FD로 열려 있지 않은지도 `/proc/self/fdinfo`로 확인합니다. 따라서 파일 크기나 수정 시각이 일정 시간 멈췄다는 이유만으로 다운로드 완료를 판정하지 않습니다. 현재보다 새로운 patch 후보가 없고 현재 Bundle 다운로드도 대기 중이 아니면 watcher thread 자체를 시작하지 않습니다. 버튼은 앱 프로세스를 완전히 종료하고, 다음 실행의 `BootstrapActivity`가 patch를 다운로드·적용한 뒤 게임을 시작합니다. 실행 중 새로 게시된 patch는 다음 실행의 index 확인 때 반영됩니다.
